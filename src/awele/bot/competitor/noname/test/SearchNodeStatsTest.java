...
public final class SearchNodeStatsTest
{
    ...
    public static void main(String[] args)
    {
        ...
        final PositionEvaluator evalA = BitMinMaxNode.positionEvaluator; // ou new PositionEvaluator(weights)
            ...
            StatsReport report)
    {
        ...
        try
        {
            ...
            while (!board.isGameOver() && movesPlayed < MAX_MOVES_PER_GAME)
            {
                ...
                BitMinMaxNode.transpositionTable = aTurn ? ttA : ttB;
    ...
    private DecisionStats getBestDecisionWithNodeStats(BitBoard board, PositionEvaluator evaluator, int depth)
    {
        ...
        if (root.isInterrupted())
            return new DecisionStats(fallbackDecision(board), BitMinMaxNode.nodeCount);

        return new DecisionStats(root.getDecision(), BitMinMaxNode.nodeCount);
    }
