...
public final class MatchSearchStatsTest
{
    ...
    public static void main(String[] args)
    {
        ...
        final PositionEvaluator evalA = BitMinMaxNode.positionEvaluator;
        final PositionEvaluator evalB = BitMinMaxNode.positionEvaluator;
            ...
            int maxDepth)
    {
        ...
        BitMinMaxNode.positionEvaluator = eval;

        // budget temps
        BitMinMaxNode.timeExpired = false;
        BitMinMaxNode.searchStartTime = System.nanoTime();
        BitMinMaxNode.maxSearchTime = timeMs * 1_000_000L;

        BitMinMaxNode.nodeCount = 0;

        BitMinMaxNode.initialize(board, maxDepth);

        BitMaxNode root = new BitMaxNode(board);

        final boolean interrupted = root.isInterrupted() || BitMinMaxNode.timeExpired;
        final double[] decision = interrupted ? fallbackDecision(board) : root.getDecision();

        return new DecisionStats(decision, BitMinMaxNode.nodeCount, interrupted);
    }
